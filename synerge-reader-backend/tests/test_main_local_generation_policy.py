"""Source/AST contract tests for the C2b local-generation policy.

These tests read and parse source files only. They never import or execute
``main.py`` and make no network, database, subprocess, or filesystem-write
...
``/api/generate`` streaming structure and failed output is not persisted.

These contracts do not validate successful Ollama protocol behavior, model
availability, mxbai, 1024 dimensions, schema compatibility, production
